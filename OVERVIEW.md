...
A `LIMIT` equal to `roundup_limit` (defaulting to `_ROUNDUP_LIMIT = 6`) caps results. After the SELECT, a single batch UPDATE writes `last_checked_at` and clears `next_check_at` for every returned row; if eligibility filtering returns zero rows, the update is skipped. The batch size is configurable via `INTERESTING_ROUNDUP_LIMIT` (parsed by `config.py`); see the Configuration section below.
